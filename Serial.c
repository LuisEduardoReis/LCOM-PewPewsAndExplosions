...
int ser_poll_send(unsigned short base_addr, unsigned long bits, unsigned long stop,	long parity, unsigned long rate, int stringc, char *strings[]) {
	...
	for(n = 0; n < stringc; n++) {
		...
		while(1) {
			...
			if (ser_send_char(base_addr, c) != 0) return 1;
			i++;
		}
		//Enviar um espaço ou ponto
		if (n >= stringc-1) {
			if (ser_send_char(base_addr, '.') != 0) return 1;
		}
		else {
			if (ser_send_char(base_addr, 32) != 0) return 1;
		}
	}
	return 0;
};
...
int ser_int_recieve(unsigned short base_addr, unsigned long bits, unsigned long stop, long parity, unsigned long rate) {
	...
	irq_set = ser_subscribe_int(base_addr);
	...
	while( 1 ) {
	    ...
	    // Serial Interrupt
	    if (is_ipc_notify(ipc_status) && _ENDPOINT_P(msg.m_source) == HARDWARE && (msg.NOTIFY_ARG & irq_set) ) {
	        /* INT HANDLER
...
int ser_poll_send_int(unsigned short base_addr, unsigned long bits, unsigned long stop,
		long parity, unsigned long rate, int val) {
	...
	int i;
	for(i = 0; i < 4; i++)
		if (ser_send_char(base_addr, c[i]) != 0) return 1;
